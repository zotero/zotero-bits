{
        ...
        "inRepository": true,
        "translatorType": 4,
        "lastUpdated": "2011-05-22 00:37:00"
}
...
function doWeb(doc, url) {
	...
	if (detectWeb(doc, url) == "multiple") {
		...
		if (true || items) {
			// maybe we have an old, single-article journal
			if (doc.evaluate('//p[@class="articleTitle"]', doc, nsResolver, XPathResult.ANY_TYPE, null).iterateNext()) {
				items[url] = "nothing";
			}
		} else {
	...
	Zotero.Utilities.processDocuments(articles, function(doc) {
		...
		if (data.match(/Posted:\s+/)) {
			item.date = Zotero.Utilities.trimInternal(data.match(/Posted:\s+(.*)\n/)[1]);
		} else {
			item.date = journalinfo[3];
		}
		item.url = doc.location.href;
		item.issue = item.url.match(/(\d+)\/S/)[1];
		var doi = doc.evaluate('//a[@href="/jourhtml/abstracthelp.html#addinfo" and contains(./text(),"DOI:")]/following-sibling::text()', doc, nsResolver, XPathResult.ANY_TYPE, null).iterateNext();
		if(doi) item.DOI = doi.textContent.trim();
