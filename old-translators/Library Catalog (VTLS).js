{
	...
	"priority":100,
	"inRepository":true,
	"lastUpdated":"2010-06-14 20:40:00"
}
...
/* Returns true if "word" is in the array for key "set" */
function inI18NSetFor(word, set) {
	var variants = {
		"Title" : 
		["Title", "Tytuł"]
	};
	if (word == set) return true;
	else if (variants[set] && variants[set].indexOf(word) !== -1) return true;
	else return false;
		...
		while(tableRow = tableRows.iterateNext()) {
			...
			if(url) {
				...
				while(field = fields.iterateNext()) {
					...
					if(inI18NSetFor(header.nodeValue,"Title")) {
