{
        ...
        "target": "^http://(books|www)\\.google\\.[a-z]+(\\.[a-z]+)?/e?books(/reader)?\\?(.*id=.*|.*q=.*)",
        "minVersion": "2.0b7",
        "maxVersion": "",
        "priority": 100,
        "inRepository": "1",
        "translatorType": 4,
        "lastUpdated": "2011-03-15 23:09:41"
}
...
Single item - eBooks URL with "id"
http://books.google.com/ebooks/reader?id=nPI2AAAAMAAJ&printsec=frontcover&output=reader
*/

function detectWeb(doc, url) {
	var re = new RegExp('^http://(books|www)\\.google\\.[a-z]+(\.[a-z]+)?/e?books(\/reader)?\\?(.*&)?(id|vid)=([^&]+)', 'i');
