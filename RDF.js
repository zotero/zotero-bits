...
var n = {
	...
	gh:"http://www.zotero.org/namespaces/ghcitation#" // XXX Hack until we can get a PURL for the citation_ stuff
};
...
function importItem(newItem, node, type) {
	...
	if (!newItem.itemType) {	
		// Google / Highwire types are implicit from specific fields
		var proceedingsTitle = getFirstResults(node, [n.gh+"conference_title"]);
		if (proceedingsTitle) newItem.itemType = "conferencePaper";
		var journalTitle = getFirstResults(node, [n.gh+"journal_title"]);
		if (journalTitle) newItem.itemType = "journalArticle";
	}	

	// title
	newItem.title = getFirstResults(node, [n.gh+"title", n.dc+"title", n.vcard2+"fn"], true);
	...
	for each(var creatorType in possibleCreatorTypes) {
		if(creatorType == "author") {
			var creators = getFirstResults(node, [n.bib+"authors", n.gh+"author", n.gh+"authors", n.dc+"creator"]);
	...
	// language
	newItem.language = getFirstResults(node, [n.gh+"language"], true);
	...
	// Handle lack of container
	if (!newItem.publicationTitle) {
		newItem.publicationTitle = getFirstResults(node, [n.gh+"journal_title", n.gh+"conference_title"], true);
	}
	...
	// volume
	newItem.volume = getFirstResults((container ? container : node), [n.prism+"volume", n.gh+"volume"], true);
	
	// issue
	newItem.issue = getFirstResults((container ? container : node), [n.prism+"number", n.gh+"issue"], true);
	...
	// publisher
	var publisher = getFirstResults(node, [n.gh+"publisher", n.dc+"publisher", n.vcard2+"org"]);
	if(publisher) {
	...
	// date
	newItem.date = getFirstResults(node, [n.gh+"date", n.dc+"date", n.dcterms+"dateSubmitted"], true);
	if (!newItem.date) {
	...
	// Google / Highwire identifiers
	var issn = getFirstResults(node, [n.gh+"issn"]);
	if (issn) newItem.ISSN = issn;
	var isbn = getFirstResults(node, [n.gh+"isbn"]);
	if (isbn) newItem.ISBN = issn;
	var doi = getFirstResults(node, [n.gh+"doi"]);
	if (doi) newItem.DOI = doi;

	// Explicit attachments from Google / Highwire
	var pdfurl = getFirstResults(node, [n.gh+"pdf_url"]);
	if(pdfurl) newItem.attachments.push({url:pdfurl, title:"Full Text PDF", mimeType:"application/pdf"});
	var htmlurl = getFirstResults(node, [n.gh+"fulltext_html_url"]);
	if(htmlurl) newItem.attachments.push({url:htmlurl, title:"Full Text HTML"});
